...
import com.hotel.exceptions.ReservationException;
import com.hotel.functions.MainFunctions;
	...
	private void processUserInput(BufferedReader reader, String userInput, RestTemplate proxy) {
		...
		try {
			...
			case "2":
				...
				for (String uri : URIS.keySet()) {
					String url = URIS.get(uri) + "/search?position={position}&size={size}&rating={rating}&datein={datein}&dateout={dateout}&price={price}";
